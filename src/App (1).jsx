...
const CUSTOM_CATS = ["غرفة نوم", "مطبخ", "انتريه", "ركنة", "تربيزة سفرة", "مكتب", "أخرى"];
const CUSTOM_STAGES = ["بانتظار البدء", "قيد النجارة", "قيد الدهان", "جاهز للتسليم", "تم التسليم"];
const STAGE_COLOR = { "بانتظار البدء": "w", "قيد النجارة": "i", "قيد الدهان": "i", "جاهز للتسليم": "s", "تم التسليم": "s" };
...
function Dashboard({ data, loading }) {
  ...
  const { invoices = [], expenses = [], products = [], customers = [], customOrders = [] } = data;
  const activeOrders = customOrders.filter(o => o.stage !== "تم التسليم");
  ...
  const kpis = [
    ...
    { l: "طلبات تصنيع جارية", v: activeOrders.length, ic: "🪚", bg: "#E0E7FF" },
  ];
  ...
  return (
    <div className="page">
      ...
      {activeOrders.length > 0 && (
        <div className="card">
          <div className="card-h"><span className="card-t">🪚 متابعة طلبات التصنيع</span></div>
          <div className="tw">
            <table>
              <thead><tr><th>رقم</th><th>العميل</th><th>الفئة</th><th>المرحلة</th><th>التسليم المتوقع</th></tr></thead>
              <tbody>
                {activeOrders.slice(0, 8).map(o => {
                  const cust = customers.find(c => c.id === o.customer_id);
                  const overdue = o.expected_delivery && o.expected_delivery < today();
                  return (
                    <tr key={o.id}>
                      <td><strong>{o.number}</strong></td>
                      <td>{cust?.name || "—"}</td>
                      <td>{o.category}</td>
                      <td><span className={`badge b-${STAGE_COLOR[o.stage] || "i"}`}>{o.stage}</span></td>
                      <td>{o.expected_delivery ? <span className={`badge b-${overdue ? "d" : "i"}`}>{overdue ? "⚠️ " : ""}{o.expected_delivery}</span> : "—"}</td>
                    </tr>
                  );
                })}
              </tbody>
...
// ===================== CUSTOM MANUFACTURING ORDERS PAGE =====================
function CustomOrdersPage({ data, reload }) {
  const { customOrders = [], customers = [] } = data;
  const [modal, setModal] = useState(false);
  const [viewModal, setViewModal] = useState(null);
  ...
  const blankForm = () => ({
    customerId: "", category: CUSTOM_CATS[0], components: [{ name: "", spec: "" }],
    price: 0, deposit: 0, cost_material: 0, cost_labor: 0, cost_paint: 0, cost_other: 0,
    stage: CUSTOM_STAGES[0], expected_delivery: "", notes: "", date: today(),
  });
  const [form, setForm] = useState(blankForm());

  const filtered = filter === "الكل" ? customOrders : customOrders.filter(o => o.stage === filter);
  const active = customOrders.filter(o => o.stage !== "تم التسليم");
  const totalRemaining = active.reduce((s, o) => s + (Number(o.price || 0) - Number(o.deposit || 0)), 0);
  const totalExpectedProfit = active.reduce((s, o) => s + (Number(o.price || 0) - (Number(o.cost_material || 0) + Number(o.cost_labor || 0) + Number(o.cost_paint || 0) + Number(o.cost_other || 0))), 0);

  const kpis = [
    { l: "طلبات قيد التنفيذ", v: active.length, ic: "🪚", bg: "#FEF3C7" },
    { l: "متبقي على العملاء", v: fmt(totalRemaining) + " ج", ic: "⏳", bg: "#FEE2E2" },
    { l: "الربح المتوقع", v: fmt(totalExpectedProfit) + " ج", ic: "💰", bg: "#DCFCE7" },
  ];

  const addComponent = () => setForm({ ...form, components: [...form.components, { name: "", spec: "" }] });
  const updComponent = (idx, k, v) => {
  ...
  const openAdd = () => { setForm(blankForm()); setEditingId(null); setModal(true); };
  const openEdit = (o) => {
    setForm({
      customerId: o.customer_id, category: o.category, components: o.components?.length ? o.components : [{ name: "", spec: "" }],
      price: o.price, deposit: o.deposit, cost_material: o.cost_material || 0, cost_labor: o.cost_labor || 0,
      cost_paint: o.cost_paint || 0, cost_other: o.cost_other || 0, stage: o.stage, expected_delivery: o.expected_delivery || "",
      notes: o.notes || "", date: o.date,
    });
    setEditingId(o.id); setModal(true);
  };

  const save = async () => {
    if (!form.customerId) return alert("اختر العميل");
    setSaving(true);
    try {
      const payload = {
        customer_id: Number(form.customerId), category: form.category,
        components: form.components.filter(c => c.name.trim()),
        price: Number(form.price || 0), deposit: Number(form.deposit || 0),
        cost_material: Number(form.cost_material || 0), cost_labor: Number(form.cost_labor || 0),
        cost_paint: Number(form.cost_paint || 0), cost_other: Number(form.cost_other || 0),
        stage: form.stage, expected_delivery: form.expected_delivery, notes: form.notes, date: form.date,
        actual_delivery: form.stage === "تم التسليم" ? (form.actual_delivery || today()) : null,
      };
      if (editingId) {
        await supabase.update("custom_orders", editingId, payload);
      } else {
        const count = customOrders.length + 1;
        payload.number = `CO-${String(count).padStart(3, "0")}`;
        await supabase.insert("custom_orders", payload);
      }
      await reload(); setModal(false); setForm(blankForm()); setEditingId(null);
    } catch (e) { alert("خطأ: " + e.message); }
    setSaving(false);
  };
  ...
  const remove = async (id) => {
    ...
    await supabase.delete("custom_orders", id); await reload();
  };

  const changeStage = async (o, stage) => {
    await supabase.update("custom_orders", o.id, { stage, actual_delivery: stage === "تم التسليم" ? today() : (o.actual_delivery || null) });
    await reload();
  };
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        <div className="card-h">
          <span className="card-t">🪚 طلبات التصنيع ({customOrders.length})</span>
          <div style={{ display: "flex", gap: 8 }}>
            <select className="fc" style={{ width: 170 }} value={filter} onChange={e => setFilter(e.target.value)}>
              <option>الكل</option>{CUSTOM_STAGES.map(s => <option key={s}>{s}</option>)}
            </select>
            <button className="btn btn-g" onClick={openAdd}>+ طلب جديد</button>
          </div>
        </div>
        <div className="tw">
          <table>
            <thead><tr><th>رقم</th><th>العميل</th><th>الفئة</th><th>المرحلة</th><th>السعر</th><th>العربون</th><th>المتبقي</th><th>التسليم المتوقع</th><th></th></tr></thead>
            <tbody>
              {filtered.map(o => {
                const cust = customers.find(c => c.id === o.customer_id);
                const remaining = Number(o.price || 0) - Number(o.deposit || 0);
                const overdue = o.expected_delivery && o.expected_delivery < today() && o.stage !== "تم التسليم";
                return (
                  <tr key={o.id}>
                    <td><strong>{o.number}</strong></td>
                    <td>{cust?.name || "—"}</td>
                    <td>{o.category}</td>
                    <td>
                      <select className="fc" style={{ fontSize: 11, padding: "4px 6px" }} value={o.stage} onChange={e => changeStage(o, e.target.value)}>
                        {CUSTOM_STAGES.map(s => <option key={s}>{s}</option>)}
                      </select>
                    </td>
                    <td>{fmt(o.price)} ج</td>
                    <td>{fmt(o.deposit)} ج</td>
                    <td><strong style={{ color: remaining > 0 ? C.danger : C.success }}>{fmt(remaining)} ج</strong></td>
                    <td>
                      {o.expected_delivery
                        ? <span className={`badge b-${overdue ? "d" : "i"}`}>{overdue ? "⚠️ " : ""}{o.expected_delivery}</span>
                        : "—"}
                    </td>
                    <td style={{ display: "flex", gap: 4 }}>
                      <button className="btn btn-o btn-sm" onClick={() => setViewModal(o)}>👁️</button>
                      <button className="btn btn-o btn-sm" onClick={() => openEdit(o)}>✏️</button>
                      <button className="btn btn-d btn-sm" onClick={() => remove(o.id)}>🗑️</button>
                    </td>
                  </tr>
                );
              })}
              {filtered.length === 0 && <tr><td colSpan={9} style={{ textAlign: "center", color: C.textLight, padding: 20 }}>لا توجد طلبات</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      {viewModal && (
        <div className="mo" onClick={e => e.target === e.currentTarget && setViewModal(null)}>
          <div className="md" style={{ maxWidth: 520 }}>
            <div className="md-h"><span className="md-t">أمر تصنيع {viewModal.number}</span><button className="mc" onClick={() => setViewModal(null)}>✕</button></div>
            <div className="md-b">
              <div className="sr"><span className="sl">العميل</span><span className="sv">{customers.find(c => c.id === viewModal.customer_id)?.name || "—"}</span></div>
              <div className="sr"><span className="sl">الفئة</span><span className="sv">{viewModal.category}</span></div>
              <div className="sr"><span className="sl">المرحلة</span><span className={`badge b-${STAGE_COLOR[viewModal.stage]}`}>{viewModal.stage}</span></div>
              <div className="sr"><span className="sl">تاريخ الطلب</span><span className="sv">{viewModal.date}</span></div>
              <div className="sr"><span className="sl">التسليم المتوقع</span><span className="sv">{viewModal.expected_delivery || "—"}</span></div>
              <hr style={{ margin: "12px 0", border: "none", borderTop: "1px solid #eee" }} />
              <strong style={{ fontSize: 13 }}>المكونات والمواصفات</strong>
              {(viewModal.components || []).map((c, i) => (
                <div key={i} className="sr"><span className="sl">{c.name}</span><span className="sv">{c.spec}</span></div>
              ))}
              <hr style={{ margin: "12px 0", border: "none", borderTop: "1px solid #eee" }} />
              <div className="sr"><span className="sl">السعر المتفق عليه</span><span className="sv"><strong>{fmt(viewModal.price)} ج</strong></span></div>
              <div className="sr"><span className="sl">العربون المدفوع</span><span className="sv">{fmt(viewModal.deposit)} ج</span></div>
              <div className="sr"><span className="sl">المتبقي</span><span className="sv"><strong style={{ color: C.danger }}>{fmt(Number(viewModal.price || 0) - Number(viewModal.deposit || 0))} ج</strong></span></div>
              <hr style={{ margin: "12px 0", border: "none", borderTop: "1px solid #eee" }} />
              <div className="sr"><span className="sl">تكلفة الخامة</span><span className="sv">{fmt(viewModal.cost_material)} ج</span></div>
              <div className="sr"><span className="sl">أجرة النجارة</span><span className="sv">{fmt(viewModal.cost_labor)} ج</span></div>
              <div className="sr"><span className="sl">تكلفة الدهانات</span><span className="sv">{fmt(viewModal.cost_paint)} ج</span></div>
              <div className="sr"><span className="sl">تكاليف أخرى</span><span className="sv">{fmt(viewModal.cost_other)} ج</span></div>
              <div className="sr"><span className="sl">صافي الربح المتوقع</span><span className="sv"><strong style={{ color: C.success }}>{fmt(Number(viewModal.price || 0) - (Number(viewModal.cost_material || 0) + Number(viewModal.cost_labor || 0) + Number(viewModal.cost_paint || 0) + Number(viewModal.cost_other || 0)))} ج</strong></span></div>
              {viewModal.notes && <div className="al al-i" style={{ marginTop: 12 }}>📝 {viewModal.notes}</div>}
            </div>
            <div className="md-f">
              <button className="btn btn-o" onClick={() => setViewModal(null)}>إغلاق</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal && (
        <div className="mo" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="md" style={{ maxWidth: 560 }}>
            <div className="md-h"><span className="md-t">{editingId ? "تعديل طلب تصنيع" : "طلب تصنيع جديد"}</span><button className="mc" onClick={() => setModal(false)}>✕</button></div>
            <div className="md-b">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div className="fg"><label className="fl">العميل</label>
                  <select className="fc" value={form.customerId} onChange={e => setForm({ ...form, customerId: e.target.value })}>
                    <option value="">اختر عميل</option>
                    {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </div>
                <div className="fg"><label className="fl">الفئة</label>
                  <select className="fc" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}>
              ...
              <div className="fg">
                <label className="fl">المكونات والمواصفات</label>
                {form.components.map((c, i) => (
                  <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6 }}>
                    <input className="fc" placeholder="اسم القطعة (سرير، دولاب...)" style={{ flex: 1 }} value={c.name} onChange={e => updComponent(i, "name", e.target.value)} />
                    <input className="fc" placeholder="المواصفات (المقاس، الخامة، اللون...)" style={{ flex: 2 }} value={c.spec} onChange={e => updComponent(i, "spec", e.target.value)} />
                    <button className="btn btn-d btn-sm" onClick={() => removeComponent(i)}>✕</button>
                  </div>
                ))}
                <button className="btn btn-o btn-sm" onClick={addComponent}>+ إضافة قطعة</button>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div className="fg"><label className="fl">السعر المتفق عليه (ج)</label><input type="number" className="fc" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} /></div>
                <div className="fg"><label className="fl">العربون المدفوع (ج)</label><input type="number" className="fc" value={form.deposit} onChange={e => setForm({ ...form, deposit: e.target.value })} /></div>
              </div>

              <div className="al al-i" style={{ fontSize: 12 }}>💡 التكاليف دي اختيارية، لو عبّيتها هيتحسبلك الربح الفعلي تلقائي.</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div className="fg"><label className="fl">تكلفة الخامة (ج)</label><input type="number" className="fc" value={form.cost_material} onChange={e => setForm({ ...form, cost_material: e.target.value })} /></div>
                <div className="fg"><label className="fl">أجرة النجارة (ج)</label><input type="number" className="fc" value={form.cost_labor} onChange={e => setForm({ ...form, cost_labor: e.target.value })} /></div>
                <div className="fg"><label className="fl">تكلفة الدهانات (ج)</label><input type="number" className="fc" value={form.cost_paint} onChange={e => setForm({ ...form, cost_paint: e.target.value })} /></div>
                <div className="fg"><label className="fl">تكاليف أخرى (ج)</label><input type="number" className="fc" value={form.cost_other} onChange={e => setForm({ ...form, cost_other: e.target.value })} /></div>
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <div className="fg"><label className="fl">المرحلة الحالية</label>
                  <select className="fc" value={form.stage} onChange={e => setForm({ ...form, stage: e.target.value })}>
                    {CUSTOM_STAGES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div className="fg"><label className="fl">تاريخ التسليم المتوقع</label><input type="date" className="fc" value={form.expected_delivery} onChange={e => setForm({ ...form, expected_delivery: e.target.value })} /></div>
              </div>
...
const NAV = [
  ...
  { s: "التصنيع", items: [{ k: "custom_orders", ic: "🪚", l: "طلبات التصنيع" }] },
...
const PT = { dashboard: "لوحة التحكم", invoices: "الفواتير والمبيعات", customers: "إدارة العملاء", custom_orders: "طلبات التصنيع", products: "إدارة المنتجات", suppliers: "إدارة الموردين", employees: "إدارة الموظفين", expenses: "إدارة المصروفات", reports: "التقارير", settings: "الإعدادات" };
...
export default function App() {
  ...
  const [data, setData] = useState({ products: [], customers: [], suppliers: [], employees: [], invoices: [], expenses: [], users: [], customOrders: [] });
  const [loading, setLoading] = useState(false);
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [products, customers, suppliers, employees, invoices, expenses, users, customOrders] = await Promise.all([
        ...
        supabase.getAll("custom_orders")
      ]);
      setData({ products, customers, suppliers, employees, invoices, expenses, users, customOrders });
      setSyncStatus("ok");
  ...
  return (
    <div>
      ...
      <div className={`main ${collapsed ? "col" : ""}`}>
        ...
        {page === "custom_orders" && <CustomOrdersPage data={data} reload={loadAll} />}
